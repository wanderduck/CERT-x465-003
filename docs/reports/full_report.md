# Pokemon Base Stats Exploratory Data Analysis — Full Report
...
## **Section 2: Type Shapes Stat Identity, Not Just Stat Total**
...
### **4b: Offense vs. Bulk — The Glass Cannon Test**

A companion scatter plot to 4a repositions the question: instead of Speed vs. Bulk, it plots Offense (Attack + Special Attack) on the x-axis against Bulk (HP + Defense + Special Defense) on the y-axis. Points are again colored by Generation on a continuous scale, with an OLS trendline overlaid and the Pearson correlation reported.

The correlation is substantially stronger than the Speed-Bulk relationship — Pearson r = 0.6096, with an OLS fit of Bulk = 0.76 × Offense + 101.7 (R² = 0.372). High-offense Pokemon tend to be *bulkier*, not frailer. The "glass cannon" trope — the idea that Pokemon with devastating Attack or Special Attack must be fragile to compensate — does not hold at the population level. As with the Speed-Bulk result in 4a, the explanation is stat-total confounding: Pokemon with high offensive stats tend to have high stat totals overall, and high stat totals fund both offense *and* defense simultaneously. The glass cannon archetype (Alakazam, Gengar, Pheromosa) exists as a deliberately designed exception, not as a statistical rule. Where the Speed-Bulk r of 0.18 left room for ambiguity, the Offense-Bulk r of 0.61 is unambiguous — offense and survivability are positively associated across the population.

### **4c: Speed-Bulk and Offense-Bulk Pearson r by Generation**
...
### **4d: Speed Tier x Stat Tier Cross-Tabulation Heatmap**
...
### **4e: Offense Tier x Stat Tier Cross-Tabulation Heatmap**
...
The fastest-growing stat is HP, with a cumulative increase of 12.8 points from Generation 1 to Generation 9. This is a notable result — HP is neither an offensive nor a defensive stat in the traditional sense, but a universal survivability resource that benefits every archetype equally. Its disproportionate growth suggests that Game Freak has favored raising the floor of how long Pokemon survive in battle rather than sharpening the offensive or defensive extremes. A higher HP pool rewards both walls (who absorb more hits) and sweepers (who can trade damage more aggressively), making it the safest stat to inflate without destabilizing the offense-defense balance documented in 5a.

Attack and Sp. Atk show clear upward trajectories as well, consistent with the franchise's increasing emphasis on distinct physical and special attackers. Defense and Sp. Def track these increases closely, reinforcing the symmetry observed in chart 5a.
...
Generation, Type 1, Type 2, Role (rule + cluster), and Legendary status — all multi-select with OR logic for roles

**Role filter (21 options)**

7 rule-based roles (`[Rule]` prefix) + 14 GMM cluster roles (`[Cluster]` prefix) in a single dropdown

**7 live KPI cards**

Count, Stat Total, Speed, BMI, Role, Legendary %, and Specialization update reactively

**6 themed tabs**

Overview, Type Identity, Power & Specialization, Generational Trends, Evolution & Roles, Pokemon Creator

**18+ interactive charts**

Bar, histogram, scatter (with OLS trendlines), radar (2c-style layered), stacked bar, and BMI comparison visualizations

**Hover tooltips**

Every data point shows individual Pokemon details on mouseover

**Dark theme**

Blue-tinted dark palette (`#1a1a2e` base) with gradient title, accented KPI cards, and glowing tab indicators

**Color coding**

Type-specific colors from the EDA's `TYPE_COLORS` palette, plus `TYPE_COLORS_LIGHT` and `TYPE_COLORS_DARK` for radar layering

**Pokemon Creator**

Full stat builder with 6 sliders (1-255), height/weight inputs, dual-type selectors, Legendary toggle, and reset button

**4 Creator comparison charts**

Radar overlay (2c-style Type 1/Type 2 layering), BST histogram with marker, BMI comparison, Speed-Bulk scatter with star marker

**GMM clustering**

14-cluster Gaussian Mixture Model (27 features: 6 z-scores + 3 scaled ratios + 18 one-hot Type 1) computed at startup

**Dual role systems**

Both rule-based (7 roles) and cluster-based (14 roles) classifications available in filters and Evolution & Roles tab
